...
Use [Configuration](/docs/guide/configuration/) to select Node or Cloudflare build targets; runtime middleware and mounts belong in `app.ts`, not in `flue.config.ts`. Deployment details are in [Deploy Agents on Node.js](/docs/deploy/node/) and [Deploy Agents on Cloudflare](/docs/deploy/cloudflare/).
